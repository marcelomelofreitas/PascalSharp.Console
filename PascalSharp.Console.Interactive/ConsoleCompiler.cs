...
namespace PascalSharp.Console.Interactive
{
	...
	{        
        ...
        public static Compiler.Compiler Compiler=null;
        ...
        public static bool ExecuteCommand(string command)
        {
            ...
            if (command.ToLower().IndexOf("resetoncompile=")==0)
            {
                ...
                System.Console.WriteLine("ResetOnCompile==" + RestartOnNewCompile);
                return true;
            }
            if (command.ToLower().IndexOf("ignorense=") == 0)
            {
                IgnoreNotSupportedError = command.IndexOf("1") > 0;
                System.Console.WriteLine("IgnoreNotSupportedError==" + IgnoreNotSupportedError);
                return true;
            }
            if (command.ToLower().IndexOf("scansubdirs=") == 0)
            {
                ScanSubdirs = command.IndexOf("1") > 0;
                System.Console.WriteLine("ScanSubdirs==" + ScanSubdirs);
                return true;
            }
            ...
            if (command.ToLower().IndexOf("rebuild=") == 0)
            {
                ...
                System.Console.WriteLine("Rebuild==" + Rebuild);
                return true;
            }
            if (command.ToLower().IndexOf("language=") == 0)
            {
                ...
                if (n < StringResourcesLanguage.AccessibleLanguages.Count && n >= 0)
                {
                    ...
                    System.Console.WriteLine("Language=" + StringResourcesLanguage.CurrentLanguageName);
                return true;
            }
            if (command == "?")
            {
                System.Console.ForegroundColor = ConsoleColor.White;
                System.Console.WriteLine(StringResourcesGet("HELP"));
                for (int i = 0; i < StringResourcesLanguage.AccessibleLanguages.Count;i++)
                    System.Console.WriteLine(string.Format("\t{0}-{1}", i, StringResourcesLanguage.AccessibleLanguages[i]));
                //int jjj = 5;
        ...
        static void StartScanDir(string mask, string directory)
        {
            ...
            System.Console.WriteLine(string.Format(StringResourcesGet("TOTAL{0}_SUCCESS{1}_PROCENT{2}"), AllFilesCount, SuccessFilesCount, Convert.ToInt32((double)SuccessFilesCount / (double)AllFilesCount * 100.0)));
        ...
        static void CompileAllFilesInDirectory(string mask,string directory)
        {
                ...
                for (int i = 0; i < GlobalErrorsList.Count; i++)
                {
                    ...
                    if (GlobalErrorsList[i] is LocatedError)
                    {
                        ...
                        if ((sl = (GlobalErrorsList[i] as LocatedError).SourceLocation) != null)
                            System.Console.WriteLine(string.Format("[{0},{1}] {2}: {3}", sl.BeginPosition.Line, sl.BeginPosition.Column, Path.GetFileName(sl.FileName), GlobalErrorsList[i].Message));
                        else
                            System.Console.WriteLine(GlobalErrorsList[i]);
                    }
                    else
                    {
            ...
            if (short_output)
                ClearLine();
            System.Console.WriteLine(string.Format(StringResourcesGet("TOTAL{0}_FILES{1}_LINES_{2}_TIME{3}"), files.Length, (length / 1024), AllLinesCompiled, (DateTime.Now - dt).TotalMilliseconds));
        }
        ...
        public static void ClearLine()
        {
            ...
            System.Console.Write(BlankString);
            System.Console.CursorLeft = 0;
        }
        ...
        public static int CompileAssembly(string FileName, CompilerOptions.OutputType outputType, bool PauseIfError)
        {
            ...
            if (IgnoreNotSupportedError)
                if ((Compiler.ErrorsList.Count == 1) && (Compiler.ErrorsList[0] is SemanticNonSupportedError))
                {
            ...
            {
                //Console.Beep();
                if (short_output) System.Console.WriteLine();
                System.Console.ForegroundColor = ConsoleColor.Gray;
            ...
            else
            {
                ...
                {
                    System.Console.ForegroundColor = ConsoleColor.White;
                    System.Console.WriteLine(string.Format(StringResourcesGet("OK_{0}MS_{1}LINES"), (DateTime.Now - ldt).TotalMilliseconds, Compiler.LinesCompiled));
                }
            }
            System.Console.ForegroundColor = ConsoleColor.Gray;
            return Compiler.ErrorsList.Count;
        }
        private static void ChangeCompilerState(ICompiler sender, CompilerState State, string FileName)
        {
            switch (State)
            {
                case CompilerState.ParserConnected:
                    System.Console.WriteLine(string.Format(StringResourcesGet("CONNECTED_PARSER{0}"),sender.ParsersController.LastParser));
                    return;
            }
            if (DetailOut)
            {
                ...
                {
                    System.Console.WriteLine(string.Format("{0}.", State));
        ...
        {
            ConsoleColor FGC = System.Console.ForegroundColor, BKC = System.Console.BackgroundColor;
            System.Console.ForegroundColor = FGColor;
            System.Console.BackgroundColor = BKColor;
            System.Console.Write(Text);
            System.Console.ForegroundColor = FGC; System.Console.BackgroundColor = BKC;
        }
        ...
        {
            System.Console.ForegroundColor = ConsoleColor.Gray;
            System.Console.WriteLine(StringResourcesGet("RESTARTING_COMPILER"));
            DateTime ldt = DateTime.Now;
            Compiler = new Compiler.Compiler(null,ChangeCompilerState);
            //GC.Collect();
            WriteColorText(PascalSharp.Compiler.Compiler.Banner + "\nCopyright (c) 2005-2018 by Ivan Bondarev, Stanislav Mihalkovich\n", ConsoleColor.Black, ConsoleColor.Green);
            System.Console.WriteLine("OK {0}ms", (DateTime.Now - ldt).TotalMilliseconds);
        ...
        public static void ShowConnectedParsers()
        {
            if (Compiler.SupportedSourceFiles.Length > 0)
            {
                System.Console.Write(StringResourcesGet("CONNECTED_PARSERS"));
                foreach (var ssf in Compiler.SupportedSourceFiles)
                    System.Console.Write(ssf+"; ");
        ...
        public static void ShowConnectedConversions()
        {
            ...
            {
                System.Console.Write(StringResourcesGet("CONNECTED_CONVERSIONS"));
                foreach (var sc in Compiler.SemanticTreeConvertersController.SemanticTreeConverters)
                    System.Console.Write(sc.Name + "; ");
                System.Console.WriteLine();
            }
        }
        ...
        public static int Main(string[] initialArgs)
		{
            ...
            System.Console.Title = StringResourcesGet("STARTING");
            Reset();
            System.Console.Title = PascalSharp.Compiler.Compiler.Banner;
            ...
            while (true)
            {
                ...
                if (!ExecuteCommand(System.Console.ReadLine()))
                {
                    System.Console.ForegroundColor = ConsoleColor.Gray;
        ...
        {
            get { return NoConsole ? DefaultConsoleBufferWidth : System.Console.BufferWidth; }
        }
	}
}
